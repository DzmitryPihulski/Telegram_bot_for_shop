...
##making tables
def make_table_Users_in_db(connection):
    try:
...
def make_table_Products_in_db(connection):
    try:
...
def make_table_Orders_in_db(connection):
    try:
...
def make_table_Admins_in_db(connection):
    try:
...
def insert_into_table_Products(*values):
...
def insert_into_table_Orders(*values):
    connection=make_connection_todb('ShopDB')
...
def delete_from_table_Products(id):
    connection=make_connection_todb('ShopDB')
...
def all_users():
    ...
    for r in rows:
        ...
        answer=answer+f"""{r.user_id}, @{r.username}, {r.first_name}, {r.last_name}\n"""
    return answer