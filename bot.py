...
def send_welcome(message):
    try:
        ...
        if cursor.execute(querry).fetchone() is None:  
            ...
            bot.send_message(message.chat.id , f"🛒 | Hey {get_user__first_name}, how are you doing? It's shop-bot here!\n Say \help and I will tell you about the commands.", reply_markup=keyboards.start_keyboard)
        else:
            bot.send_message(message.chat.id, f"⛔️ | You have already registrated. Say \help and I will tell you about the commands.",reply_markup=keyboards.start_keyboard)
    except pyodbc.Error as ex:
        bot.send_message(message.chat.id, f'🚫 | Error in command',reply_markup=keyboards.start_keyboard)
        bot.send_message(functions_for_db.admin_chat('dimapiggy'), f"Hi! We have a problem in /start command with user\n Details:\n User username @{get_user_username}\n User first and last names {get_user__first_name} {get_user_last_name}\n The error: {ex}",reply_markup=keyboards.admin_keyboard)
...
def greetings(message):
    ...
    bot.send_message(message.chat.id, text,reply_markup=keyboards.start_keyboard)

#Check my data
@bot.message_handler(commands=['id'])
def id(message):
    bot.send_message(message.chat.id, 'Your username: '+str(message.from_user.username)+'.\nYour user id: '+str(message.from_user.id)+'.\nYour chat id: '+str(message.chat.id),reply_markup=keyboards.start_keyboard)
...
def view_all_products(message):
    a=functions_for_db.all_awailable_products()
    for i in a:
        bot.send_message(message.chat.id,i[0],parse_mode='Markdown')
...
def get_admin(message):
    if is_admin(message)==False:
        sent_msg=bot.send_message(message.chat.id, f'Wooo {message.from_user.username}👻. I see you want to get access to admins!\n It is not so easy. Firstly I want you to give password.🛂')
        bot.register_next_step_handler(sent_msg, get_admin_status)
    else:
        bot.send_message(message.chat.id, f"⛔️ | You have already registrated. Say \help and I will tell you about the commands.",reply_markup=keyboards.admin_keyboard)
...
def get_admin_status(message):
    if message.text==passwords.PASSWORDS:
        ...
        bot.send_message(message.chat.id, f'Congratulations {message.from_user.username}. You are now admin!',reply_markup=keyboards.admin_keyboard)
    else:
        bot.send_message(message.chat.id, f'Sorry {message.from_user.username}. It is not an aceptable answer!',reply_markup=keyboards.start_keyboard)
...
def make_db(message):
    bot.send_message(message.chat.id, functions_for_db.make_db('ShopDB'),reply_markup=keyboards.start_keyboard)

#make tables
@bot.message_handler(commands=['make_tables'])
def make_db(message):
    bot.send_message(message.chat.id, functions_for_db.make_tables(functions_for_db.make_connection_todb('ShopDB')),reply_markup=keyboards.start_keyboard)

#View all products from SQL Products table
@bot.message_handler(commands=['all_products'])
def view_all_products(message):
    if is_admin(message)==True:
        a=functions_for_db.all_products()
        for i in a:
            bot.send_message(message.chat.id,i[0],parse_mode='Markdown')
...
def add_product(message):
    if is_admin(message)==True:
        ...
        bot.register_next_step_handler(sent_msg, functions_for_db.insert_into_table_Products)

#Change price of certain product in SQL Products table
@bot.message_handler(commands=['change_product_price'])
def change_price_of_prt(message):
    if is_admin(message)==True:
        sent_msg=bot.send_message(message.chat.id, f'Give me product id and new price in like that: product_id newprice')
        bot.register_next_step_handler(sent_msg, change_price_of_product)
...
def change_awailable_of_prt(message):
    if is_admin(message)==True:
        sent_msg=bot.send_message(message.chat.id, f'Give me product id and new status(0=False, 1=True) in like that: product_id newstatus. For example: 3 0')
        bot.register_next_step_handler(sent_msg, change_awailable_of_product)
...
def add_product(message):
    if is_admin(message)==True:
        ...
        sent_msg=bot.send_message(message.chat.id, text)
        bot.register_next_step_handler(sent_msg, functions_for_db.delete_from_table_Products)

#View all users of our bot from SQL Users table
@bot.message_handler(commands=['all_users'])
def view_all_users(message):
    if is_admin(message)==True:
        a=functions_for_db.all_users()
        bot.send_message(message.chat.id,a,parse_mode='Markdown',reply_markup=keyboards.admin_keyboard)

#If it is not a command, then just return the input(echo bot for any other inputs, not commands)
@bot.message_handler(func=lambda msg: True)
def echo_all(message):
    bot.reply_to(message, message.text,reply_markup=keyboards.start_keyboard)
...
def change_price_of_product(message):
    ...
    try:
        ...
        bot.send_message(message.chat.id,f"The error '{ex}' occurred")

def change_awailable_of_product(message):
    arguments=list(message.text.split(" "))
    connection=functions_for_db.make_connection_todb('ShopDB')
...
def make_order_3(adress, product_name_and_amount):
    functions_for_db.insert_into_table_Orders([int(adress.from_user.id),adress.from_user.username,functions_for_db.get_id_of_product(product_name_and_amount[0]),int(product_name_and_amount[1]),str(adress.text),datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")])
    bot.send_message(adress.chat.id, f'Your order is placed',reply_markup=keyboards.start_keyboard)
    bot.send_message(functions_for_db.admin_chat('dimapiggy'), f"Hi!\n We have new order!\n Details:\n Username @{adress.from_user.username}\n Product name {product_name_and_amount[0]}\n Amount {product_name_and_amount[1]}\n Adress {str(adress.text)}\n Date {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
